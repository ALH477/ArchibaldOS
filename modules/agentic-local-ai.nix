...
# - Tiered presets with automatic VRAM detection and recommendations
# - Comprehensive monitoring, health checks, and graceful degradation
# - Zero-trust security model: localhost-only, minimal privileges, read-only where possible
# - Extensive validation, error handling, and operational tooling
...
let
  ...
  # Prometheus metrics service (optional)
  prometheusService = optionalString cfg.advanced.monitoring.enable ''
      prometheus:
    ...
    case "''${1:-}" in
      start|up)
        ...
        if health_check; then
          ...
          ${optionalString cfg.advanced.monitoring.enable ''success "Metrics: http://${cfg.network.ollamaBindAddress}:9090"''}
          echo ""
...
in
{
  options.archibaldos.profiles.ai.agenticLocalAi = {
    ...
    preset = mkOption {
      ...
      description = mdDoc ''
        ...
        - **cpu-fallback**: CPU-only, minimal resources (no GPU required)
        - **default**: Consumer GPUs with 8-24GB VRAM
        - **high-vram**: High-end GPUs with 40GB+ VRAM
        - **pewdiepie**: Multi-GPU extreme setups (320GB+ VRAM, CUDA required)
    ...
    preloadModels = mkOption {
      ...
      description = mdDoc ''
        ...
        Useful for ensuring critical models are always available.
      '';
    };
  ...
  config = mkIf cfg.enable {
    ...
    assertions = [
      ...
      {
        assertion = effectiveAcceleration != "rocm" || (pathExists "/dev/kfd" || pathExists "/dev/dri");
        message = "ROCm acceleration selected but /dev/kfd or /dev/dri not present. Ensure ROCm drivers are installed.";
      }
      {
        assertion = effectiveAcceleration != "cuda" || (config.hardware.nvidia.modesetting.enable or false) || (config.hardware.nvidia-container-toolkit.enable or false);
        message = "CUDA acceleration requires NVIDIA drivers. Enable hardware.nvidia.modesetting or ensure nvidia-docker is configured.";
      }
      {
      ...
      {
        assertion = all (model: builtins.match "^[a-z0-9._:-]+$" model != null) cfg.preloadModels;
        message = "Model names must only contain lowercase letters, numbers, dots, colons, hyphens, and underscores.";
      }
    ];
    ...
    # Missing autoStart option declaration - add it
    advanced.autoStart = mkEnableOption "Automatic startup of AI stack on boot";
    ...
    # Firewall configuration (if network exposure is enabled)
    ...
    system.extraSystemBuilderCmds = ''
      ...
      Network Configuration:
        ...
        ${optionalString cfg.advanced.monitoring.enable "Prometheus: http://${cfg.network.ollamaBindAddress}:9090"}
