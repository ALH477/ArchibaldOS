  ...
  config = mkIf cfg.enable {
    ...
    assertions = [
      ...
      }
      {
        ...
        message = "ROCm acceleration selected but /dev/kfd not present. Ensure ROCm drivers are installed.";
      }
      {
        assertion = effectiveAcceleration != "cuda" || config.hardware.nvidia.modesetting.enable or false;
        message = "CUDA acceleration requires NVIDIA drivers. Enable hardware.nvidia.modesetting.";
      }
      {
    ...
    warnings = [
      ...
      (mkIf (cfg.preset == "pewdiepie" && currentPreset.minVramGB > 0)
    ...
    networking.firewall = mkIf (cfg.network.webUIBindAddress != "127.0.0.1" || cfg.network.ollamaBindAddress != "127.0.0.1") {
      allowedTCPPorts = 
