...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    assertions = [
      ...
      {
        assertion = effectiveAcceleration != "cuda" || 
          (pathExists "/dev/nvidia0" || pathExists "/dev/nvidiactl" || 
           (config.hardware.nvidia.modesetting.enable or false) || 
           (config.hardware.nvidia-container-toolkit.enable or false));
        message = "CUDA acceleration requires NVIDIA drivers and devices. Enable hardware.nvidia.modesetting or ensure nvidia-docker is configured.";
      }
      {
    ...
    systemd.services.agentic-ai-stack = mkIf (cfg.advanced.autoStart or false) {
      ...
      serviceConfig = {
        ...
        TimeoutStartSec = 600;  # 10 minutes for image pulls
        TimeoutStopSec = 120;
        Restart = "on-failure";
        RestartSec = 30;
      };
